...
(defun svg-display-drawing (filepath)
  ...
  (let ((command
    ...
    #+sbcl (sb-ext:run-program command
                               (list (namestring filepath))
                               :wait nil)
    #+openmcl (ccl::os-command (format nil "~a ~a" command filepath)))
  filepath)

(defun write-and-show-svg (drawing directory filename)
  "Write and display the drawing as an svg file."
  (let ((filepath (svg-write-drawing drawing directory filename)))
    (log-info "Viewing drawing as svg.")
    (svg-display-drawing filepath)
    filepath))
